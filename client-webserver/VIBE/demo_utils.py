...
def download_url(url, outdir):
    ...
    cmd = ['wget', '-c', url, '-P', outdir, '--no-check-certificate']
    subprocess.call(cmd)
...
def download_ckpt(outdir='data/vibe_data', use_3dpw=False):
    ...
    if use_3dpw:
        ckpt_file = os.path.join(VIBE_DIR, 'data/vibe_data/vibe_model_w_3dpw.pth.tar')
        url = 'https://www.dropbox.com/s/41ozgqorcp095ja/vibe_model_w_3dpw.pth.tar'
        if not os.path.isfile(ckpt_file):
            download_url(url=url, outdir=outdir)
    else:
        ckpt_file = os.path.join(VIBE_DIR, 'data/vibe_data/vibe_model_wo_3dpw.pth.tar')
